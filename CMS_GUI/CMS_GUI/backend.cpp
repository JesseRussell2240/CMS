...
void BackEnd::recordAudio() {
    ...
    qDebug("C/C++ record audio function called");
     ...
     RecordAudioTB(iBigBuf, lBigBufSize, 2, 8000);
}
...
void BackEnd::saveAudio() {
    // Call the save audio function
    qDebug("C/C++ save audio function called");

    QString filePath = QFileDialog::getSaveFileName(nullptr, "Save Audio File", "C://", "Audio Files (*.wav)");

    if (!filePath.isEmpty()) {
        // The user selected a file; you can now save the audio to this file.
        qDebug("File selected for saving: %s", filePath.toLatin1().data());
        ...
        if (file.open(QIODevice::WriteOnly)) {
            ...
            // file.write(audioData, audioDataSize);
...
void BackEnd::loadAudio() {
    ...
    // Call the load audio function
    QString filePath = QFileDialog::getOpenFileName(nullptr, "Load Audio File", "C://", "Audio Files (*.wav)");
    if (!filePath.isEmpty()) {
        // The user selected a file, you can now load the audio from this file.
        qDebug("File selected for loading: %s", filePath.toLatin1().data());
        // Add your loading logic here using the 'filePath'.
    }
